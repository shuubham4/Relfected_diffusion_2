...
class EulerMaruyamaPredictor(Predictor):
    ...
    def update_fn_s(self, x, t, *args, **kwargs):
        dt = -1. / self.rsde.N
    ...
    #def update_fn(self, x, t, s, *args, **kwargs):
    def update_fn(self, x, t, s, *args):
        dt = -1. / self.rsde.N
        z = torch.randn_like(x)
        #f, g = self.rsde.sde(x, t, *args, **kwargs)
        #f, g = self.sde.sde(x, t, *args, **kwargs)
        f, g = self.sde.sde(x, t, s, *args)
        x_mean = x + f * dt
        if g.ndim < x.ndim:
            g = g.view( *g.size(), *((1,)*(x.ndim - g.ndim)) )
        x = x_mean + g * np.sqrt(-dt) * z
