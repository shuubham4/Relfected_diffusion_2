...
class ScoreModel(pl.LightningModule):
    ...
    #     num_eval_files: int = 1, loss_type: str = 'mse', data_module_cls = None, **kwargs
    # ):
    def __init__(self,
        backbone: str = "blade", sde: str = "dfnet_sde_2", sde_s: str = "ouve_s",
        lr: float = 1e-4, ema_decay: float = 0.999,
        t_eps: float = 3e-2, transform: str = 'none', nolog: bool = False,
        num_eval_files: int = 1, loss_type: str = 'mse', data_module_cls = None, **kwargs
    ):
        ...
        self.sde = sde_cls(1.5,1.5,**kwargs)
        sde_s_cls = SDERegistry.get_by_name("ouve_s")
        self.sde_s = sde_s_cls(1.5,1.5,**kwargs)
    ...
    def get_pc_sampler(self, predictor_name, corrector_name, y, N=None, minibatch=None, scale_factor=None, **kwargs):
        ...
        if minibatch is None:
            return sampling.get_pc_sampler(predictor_name, corrector_name, sde=sde, sde_s=self.sde_s, score_fn=self, y=y, **kwargs)
            #return sampling.get_pc_sampler(predictor_name, corrector_name, sde, self, y, x, **kwargs)
        else:
            M = y.shape[0]
            def batched_sampling_fn():
                ...
                for i in range(int(ceil(M / minibatch))):
                    ...
                    sampler = sampling.get_pc_sampler(predictor_name, corrector_name, sde=sde, sde_s=self.sde_s, score_fn=self, y=y_mini, **kwargs)
                    #sampler = sampling.get_pc_sampler(predictor_name, corrector_name, sde, self, y_mini, x, **kwargs)
                    sample, n = sampler()
    ...
    # def enhance(self, y, x, sampler_type="pc", predictor="reverse_diffusion",
    #     corrector="ald", N=50, corrector_steps=1, snr=0.5, timeit=False,
    #     scale_factor = None, return_stft=False,
    #     **kwargs
    # ):

    def enhance(self, y, sampler_type="pc", predictor="euler_maruyama",
        corrector="ald", N=50, corrector_steps=1, snr=0.5, timeit=False,
        scale_factor = None, return_stft=False,
        **kwargs
    ):    
        ...
        if sampler_type == "pc":
            # sampler = self.get_pc_sampler(predictor, corrector, Y, X, N=N,
...
class StochasticRegenerationModel(pl.LightningModule):
    def __init__(self,
        ...
        lr: float = 1e-4, ema_decay: float = 0.999,
        t_eps: float = 3e-2, nolog: bool = False, num_eval_files: int = 2,
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--ema_decay", type=float, default=0.999, help="The parameter EMA decay constant (0.999 by default)")
        parser.add_argument("--t_eps", type=float, default=0.03, help="The minimum time (3e-2 by default)")
        parser.add_argument("--num_eval_files", type=int, default=2, help="Number of files for speech enhancement performance evaluation during training.")
    ...
    def validation_step(self, batch, batch_idx, discriminative=False, sr=16000):
        ...
        if batch_idx == 0 and self.num_eval_files != 0:
            ...
            if audio is not None:
                ...
                for idx, (y, x_hat, x) in enumerate(zip(y_list, x_hat_list, x_list)):
                    if self.current_epoch == 0:
                        self.logger.experiment.add_audio(f"Epoch={self.current_epoch} Mix/{idx}", (y / torch.max(torch.abs(y))).unsqueeze(1), sample_rate=sr, global_step=0)
                        self.logger.experiment.add_audio(f"Epoch={self.current_epoch} Clean/{idx}", (x / torch.max(x)).unsqueeze(1), sample_rate=sr, global_step=0)
                    self.logger.experiment.add_audio(f"Epoch={self.current_epoch} Estimate/{idx}", (x_hat / torch.max(torch.abs(x_hat))).unsqueeze(1), sample_rate=sr, global_step=0)
