...
class SDE(abc.ABC):
    ...
    #def discretize(self, x, x0, t, *args):
    def discretize(self, x, t, *args):
        ...
        """
        dt = 1 / self.N
        #drift, diffusion = self.sde(x, x0, t, *args)
    ...
    def reverse(oself, score_model, probability_flow=False, diffusion_power_gradient=None):
        ...
        class RSDE(oself.__class__):
            ...
            #def sde(self, x, x0, t, *args, **kwargs):
            def sde(self, x, t, *args, **kwargs):
                """Create the drift and diffusion functions for the reverse SDE/ODE."""
                #rsde_parts = self.rsde_parts(x, x0, t, *args, **kwargs)
            ...
            #def rsde_parts(self, x, x0, t, *args, **kwargs):
            def rsde_parts(self, x, t, *args, **kwargs):
                #sde_drift, sde_diffusion = sde_fn(x, x0, t, *args)
            ...
            #def discretize(self, x, x0, t, *args, **kwargs):
            def discretize(self, x, t, *args, **kwargs):
                """Create discretized iteration rules for the reverse diffusion sampler."""
                #f, G = discretize_fn(x, x0, t, *args)
                ...
                if "conditioning" in kwargs.keys() and kwargs["conditioning"] is not None:
                    ...
                    rev_f = f - G ** 2 * score_model(x, t, args[0]) * (0.5 if self.probability_flow else 1.)
...
# @SDERegistry.register("dfnet_sde")
# class ReflOUVESDE(SDE):
#     def __init__(self, gamma1, gamma2, sigma_min, sigma_max, N=1000, **ignored_kwargs):
#         """Construct an Ornstein-Uhlenbeck Variance Exploding SDE.
...
#     def copy(self):
#         return ReflOUVESDE(self.gamma1, self.gamma2, self.sigma_min, self.sigma_max, N=self.N)

#     @property
#     def T(self):
#         return 1

#     #def sde(self, x, x0, t, y):
#     def sde(self, x, t, y, x0):
#         sigma_min, sigma_max, logsig = self.sigma_min, self.sigma_max, self.logsig
#         gamma1 = self.gamma1
#         gamma2 = self.gamma2        
#         drift = (y * (1 - gamma1 * torch.exp(-gamma1 * t)[:, None, None, None]) + gamma1 * torch.exp(-gamma1 * t)[:, None, None, None] * x0 - x) * gamma2
#         diffusion = torch.sqrt((gamma1 ** 2) * (sigma_min**2 * torch.exp(-2 * gamma1 * t) * (torch.exp(2 * (gamma1 + logsig) * t) - 1) * logsig)/((gamma1 + logsig) ) + 4* (sigma_min ** (2*(1-t))) * (sigma_max ** (2*t)) * logsig) 
#         return drift, diffusion

#     def _mean(self, x0, t, y):
#         gamma1 = self.gamma1
#         gamma2 = self.gamma2
#         mean = y * (1 - gamma1 * torch.exp(-gamma1 * t)[:, None, None, None]) + gamma1 * torch.exp(-gamma1 * t)[:, None, None, None] * x0
#         return mean
...
#         gamma1 = self.gamma1
#         gamma2 = self.gamma2
#         var1 = (gamma1 ** 2) * (sigma_min**2 * torch.exp(-2 * gamma1 * t) * (torch.exp(2 * (gamma1 + logsig) * t) - 1) * logsig)/((gamma1 + logsig) * 2 * gamma2)
#         var2 =  2* (sigma_min ** (2*(1-t))) * (sigma_max ** (2*t)) * logsig / gamma2
#         var = var1 + var2   
#         return var
...
#         parser.add_argument("--sigma-max", type=float, default=0.5, 
#             help="The maximum sigma to use.")
#         parser.add_argument("--gamma1", type=float, default=1.5, 
#             help="gamma1.")
#         parser.add_argument("--gamma2", type=float, default=1.5, 
#             help="gamma2.")                        
#         return parser
...
@SDERegistry.register("dfnet_sde_2")
class ReflOUVESDE2(SDE):
    def __init__(self, gamma1, gamma2, sigma_min, sigma_max, N=1000, **ignored_kwargs):
        """Construct an Ornstein-Uhlenbeck Variance Exploding SDE.
    ...
    def copy(self):
        return ReflOUVESDE2(self.gamma1, self.gamma2, self.sigma_min, self.sigma_max, N=self.N)
    ...
    def _std(self, t, **kwargs):
        # This is a full solution to the ODE for P(t) in our derivations, after choosing g(s) as in self.sde()
        sigma_min, sigma_max, gamma1, logsig = self.sigma_min, self.sigma_max, self.gamma1, self.logsig
        S_g = (sigma_min**2) * ((sigma_max/sigma_min) ** (2*1) - 1)  
        S_sig = ((sigma_min**2) / (gamma1 + logsig)) * (((sigma_max/sigma_min) ** (2*1) - 1)/(2*logsig) + (torch.exp(-2 * gamma1 * 1)-1)/(2*gamma1))
        g2 =  2 * (sigma_min ** (2*(1-1))) * (sigma_max ** (2*1)) * logsig
        # could maybe replace the two torch.exp(... * t) terms here by cached values **t
        return torch.sqrt(1/g2 + 1/(S_g + S_sig))



    def sde(self, x, t, s, y, score):
        sigma_min, sigma_max, logsig = self.sigma_min, self.sigma_max, self.logsig
        gamma1 = self.gamma1
        gamma2 = self.gamma2 
        N = self.N       
        S_g = (sigma_min**2) * ((sigma_max/sigma_min) ** (2*t) - 1)  
        S_sig = ((sigma_min**2) / (gamma1 + logsig)) * (((sigma_max/sigma_min) ** (2*t) - 1)/(2*logsig) + (torch.exp(-2 * gamma1 * t)-1)/(2*gamma1))
        g2 =  2 * (sigma_min ** (2*(1-t))) * (sigma_max ** (2*t)) * logsig
        sig2 =  ((sigma_min**2) * torch.exp(-2 * gamma1 * t) * (torch.exp(2 * (gamma1 + logsig) * t) - 1) * logsig) / (gamma1 + logsig)
        diffusion = ((S_g + S_sig) * g2)/(g2 + S_g + S_sig)
        #drift = g2/(g2 + S_g + S_sig) - diffusion * (1/g2 - (t-2/N)/(S_g+S_sig)*y) - s*(-1/g2 + (1-torch.exp(torch.tensor(-gamma1 * t)))/((torch.exp(torch.tensor(-gamma1))-1)*(S_g + S_sig))) - score(s,t,torch.zeros_like(y))*(sig2/(S_g+S_sig)) * diffusion * ((1-torch.exp(torch.tensor(-gamma1 * t)))/(torch.exp(torch.tensor(-gamma1))-1))
        drift = x - g2/(g2 + S_g + S_sig) * ((x + sig2*score(s,t,torch.zeros_like(y)) - y) * ((torch.exp(torch.tensor(-gamma1 * t)) - 1)/gamma1) - (t-2/N)*y)
        return drift, diffusion        
...
class OUVE_S(SDE):
    ...
    def copy(self):
        return OUVE_S(self.gamma1, self.gamma2, self.sigma_min, self.sigma_max, N=self.N)
    ...
    def _std(self, t, **kwargs):
        # This is a full solution to the ODE for P(t) in our derivations, after choosing g(s) as in self.sde()
        sigma_min, gamma1, logsig = self.sigma_min, self.gamma1, self.logsig
        # could maybe replace the two torch.exp(... * t) terms here by cached values **t
        return torch.sqrt(
            (
                sigma_min**2
                * torch.exp(-2 * gamma1 * t)
                * (torch.exp(2 * (gamma1 + logsig) * t) - 1)
                * logsig
            )
            /
            (gamma1 + logsig)
